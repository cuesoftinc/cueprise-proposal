/* ==========================================
   STACO Insurance Demo - Interactive JavaScript
   ========================================== */
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function initLossRatioChart() {
    ...
    new Chart(ctx, {
      type: 'doughnut',
      data: {
        labels: ['Settled', 'Outstanding', 'IBNR'],
        datasets: [{
          data: [62, 25, 13],
  ...
  function initAnalyticsCharts() {
    ...
    if (segmentCtx) {
      new Chart(segmentCtx, {
        type: 'doughnut',
        data: {
          labels: ['Underwriting', 'Claims', 'Brokerage'],
          datasets: [{
            data: [72, 18, 10],
  ...
  // Demo notification
  console.log('🚀 STACO Insurance Demo loaded successfully');
...
// Form Handlers
// ==========================================
function handlePolicyQuoteSubmit(e) {
  e.preventDefault();
  const form = e.target;
  const formData = new FormData(form);

  showToast('Policy Quote Created', `New policy quote for ${formData.get('policyholder')} submitted successfully.`, 'success');
  closeModal('policy-quote-modal');
  form.reset();
}
...
// Action Handlers
// ==========================================
function bindPolicyQuote(id) {
  showToast('Quote Bound', `Policy Quote ${id} has been bound and converted to an active policy.`, 'success');
}
...
function handleInvoiceSubmit(e) {
  ...
  document.getElementById('invoice-items-container').innerHTML = `
        <div class="invoice-item" data-item="1">
            <div class="form-group" style="flex: 2;">
                <input type="text" class="form-input" name="item_desc_1" placeholder="Description (e.g. Policy premium - January)" required>
            </div>
            <div class="form-group">
                <input type="number" class="form-input item-volume" name="item_volume_1" placeholder="Premium (₦ Millions)" oninput="updateInvoiceTotal()" required>
            </div>
            <div class="form-group">
                <input type="number" class="form-input item-rate" name="item_rate_1" placeholder="Rate (₦)" oninput="updateInvoiceTotal()" required>
            </div>
            <div class="form-group item-subtotal">
                <span class="subtotal-label">₦0</span>
            </div>
...
function generateInvoice(id) {
  showToast('Invoice Generated', `Invoice for policy quote ${id} has been created.`, 'success');
}
...
function switchChartPeriod(button, period) {
  ...
  showToast('Chart Updated', `Showing ${periodLabels[period]} volume validation data.`, 'info');
...
// ==========================================
function handleFnolSubmit(e) {
  e.preventDefault();
  const form = e.target;
  const formData = new FormData(form);
  const claimId = 'CLM-2026-' + String(Math.floor(Math.random() * 9000) + 1000);
  const productLine = formData.get('productLine') || 'Property All Risk';
  const policyholder = formData.get('policyholder') || 'Policyholder';

  showToast('FNOL Submitted', `${claimId} (${productLine} for ${policyholder}) routed to triage.`, 'success');
  closeModal('fnol-modal');
  form.reset();
}
